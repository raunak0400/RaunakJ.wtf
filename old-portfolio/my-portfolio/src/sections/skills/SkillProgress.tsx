...
export default function SkillProgress({ skills, activeCategory }: SkillProgressProps) {
  ...
  return (
    <div className="mb-8 p-6 bg-gradient-to-br from-slate-900/80 to-slate-800/80 rounded-3xl border border-white/10 shadow-xl">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
        ...
        >
          <div className="text-4xl font-extrabold text-cyan-400 mb-2 drop-shadow-lg">{averageLevel}%</div>
          <div className="text-base text-gray-200 font-semibold">Average Proficiency</div>
          <div className="mt-3">
        ...
        <motion.div
          ...
          className="text-center"
        >
          <div className="text-4xl font-extrabold text-green-400 mb-2 drop-shadow-lg">{filteredSkills.length}</div>
          <div className="text-base text-gray-200 font-semibold">Total Skills</div>
        ...
        <motion.div
          ...
          className="text-center"
        >
          <div className="text-3xl font-extrabold text-purple-300 mb-1 drop-shadow-lg">
            {activeCategory === 'all' ? 'All' : activeCategory}
          </div>
          <div className="text-base text-gray-200 mb-3">Current Category</div>
          <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-cyan-700/40 scrollbar-track-transparent px-2">
            <div className="flex gap-6 min-w-[600px] justify-center">
              {CATEGORIES.map((cat) => {
                const count = skills.filter(skill => skill.category === cat).length;
                const total = skills.length;
                const percentage = total > 0 ? (count / total) * 100 : 0;
                return (
                  <div key={cat} className="flex flex-col items-center group w-20">
                    <div 
                      className="w-2 h-12 md:h-16 rounded-full bg-gray-700 relative overflow-hidden mb-1"
                    ...
                    >
                      ...
                      />
                      {/* Tooltip */}
                      <span className="absolute left-1/2 -translate-x-1/2 -top-7 scale-0 group-hover:scale-100 transition-transform bg-black/80 text-xs text-white px-2 py-1 rounded shadow-lg z-20 whitespace-nowrap">
