...
export default function Education() {
  return (
    <section id="education" className="relative min-h-screen flex flex-col justify-center py-16 bg-black overflow-hidden">
      {/* Animated Background */}
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Timeline */}
        <div className="relative flex flex-col gap-10 before:content-[''] before:absolute before:left-1/2 before:top-0 before:bottom-0 before:w-1 before:bg-gradient-to-b before:from-cyan-400/40 before:to-fuchsia-400/40 before:-translate-x-1/2">
          {educationData.map((edu, i) => (
            <motion.div
              key={edu.title}
              className={`relative flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-10 px-4 py-8 bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl ${i % 2 === 0 ? 'md:self-start' : 'md:self-end'}`}
              initial="offscreen"
            ...
            >
              <div className="flex flex-col items-center md:items-start gap-2 min-w-[60px]">
                <span className="text-3xl">{edu.icon}</span>
                <span className="block w-2 h-2 rounded-full bg-gradient-to-br from-cyan-400 to-fuchsia-400 animate-pulse" />
              </div>
              <div className="flex-1 text-left">
                <h3 className="text-2xl font-bold text-white mb-1">{edu.title}</h3>
                <p className="text-gray-300 mb-2 text-lg">{edu.description}</p>
                <p className="text-gray-400 mb-2 text-sm font-mono">{edu.year}</p>
                {edu.pdf && (
                  <a
                    href={edu.pdf}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-4 py-2 mt-2 bg-gradient-to-r from-cyan-500 to-fuchsia-500 text-white rounded-full font-semibold shadow-lg hover:from-cyan-400 hover:to-fuchsia-400 transition-all duration-300"
                  >
