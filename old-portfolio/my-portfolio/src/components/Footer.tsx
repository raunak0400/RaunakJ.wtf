...
export default function Footer() {
  ...
  return (
    <footer className="relative w-full py-12 px-4 bg-gradient-to-br from-black via-gray-900 to-black/90 backdrop-blur-xl border-t-0 mt-16 overflow-hidden shadow-2xl">
      ...
      <div className="relative z-10 max-w-4xl mx-auto flex flex-col items-center justify-center gap-6 text-center">
        {/* Copyright */}
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, type: 'spring' }}
          className="text-gray-300 text-lg font-semibold tracking-wide flex items-center justify-center gap-2"
        >
          <span className="text-2xl">&copy;</span> <span>Raunak Kumar Jha</span>
        </motion.div>
        ...
        >
          <span className="flex items-center gap-2 text-xl md:text-2xl font-bold bg-gradient-to-r from-cyan-400 via-fuchsia-400 to-blue-400 bg-clip-text text-transparent animate-gradient-x">
            <motion.span
            ...
            Made with love by a passionate programmer Raunak Kumar Jha
          </span>
        </motion.div>
        {/* Socials & WhatsApp */}
        <motion.div
          ...
          className="flex flex-wrap justify-center gap-8 mt-2 md:mt-0"
        >
          {socials.map((s, i) => (
            <motion.div
              key={s.label}
            ...
            >
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                className="transition-transform duration-200 text-4xl md:text-4xl lg:text-5xl"
                aria-label={s.label}
              ...
              <AnimatePresence>
                {hovered === i && (
                  <motion.span
                    ...
                    animate="hover"
                    exit="exit"
                    className="mt-2 text-sm font-semibold text-white bg-black/80 px-3 py-1 rounded-full shadow-lg"
                  >
                    {s.label}
                  </motion.span>
                )}
      ...
      <AnimatePresence>
        {showTop && (
          <motion.button
            ...
            onClick={scrollToTop}
            className="fixed bottom-8 right-8 z-50 bg-gradient-to-r from-cyan-500 to-blue-600 text-white p-4 rounded-full shadow-xl hover:from-cyan-400 hover:to-blue-500 focus:outline-none focus:ring-4 focus:ring-cyan-400/50 flex flex-col items-center group"
            aria-label="Back to top"
          >
            <FaChevronUp className="text-2xl animate-bounce" />
